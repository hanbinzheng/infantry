...
DbusData dbus_data;
uint32_t dbus_tick;
// DMA1/2 cannot access DTCM for STM32H7 series, here change to D2SRAM
// reference: 
...
void dbus_data_interpret(uint8_t * buff, DbusData *dbus_data) 
{
    ...
    dbus_data->rs_x = (((buff[0] | (buff[1] << 8)) & 0x07FF) - CHANNEL_OFFSET) / CHANNEL_RATIO;
    dbus_data->rs_y = ((((buff[1] >> 3) | (buff[2] << 5)) & 0x07FF) - CHANNEL_OFFSET) / CHANNEL_RATIO;
    dbus_data->ls_x = ((((buff[2] >> 6) | (buff[3] << 2) | (buff[4] << 10)) & 0x07FF) - CHANNEL_OFFSET) / CHANNEL_RATIO;
    dbus_data->ls_y = ((((buff[4] >> 1) | (buff[5] << 7)) & 0x07FF) - CHANNEL_OFFSET) / CHANNEL_RATIO;
